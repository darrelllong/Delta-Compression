...
type DiffFn = fn(&[u8], &[u8], usize, usize, bool, bool) -> Vec<Command>;

fn roundtrip(algo_fn: DiffFn, r: &[u8], v: &[u8], p: usize) -> Vec<u8> {
    let cmds = algo_fn(r, v, p, TABLE_SIZE, false, false);
...
fn correcting_wrapper(r: &[u8], v: &[u8], p: usize, q: usize, _verbose: bool, use_splay: bool) -> Vec<Command> {
    diff_correcting(r, v, p, q, 256, false, use_splay)
}
...
fn test_paper_example() {
    ...
    for (name, algo) in all_algos() {
        assert_eq!(apply_delta(r, &algo(r, v, 2, TABLE_SIZE, false, false)), v, "failed for {}", name);
    }
}
...
fn test_identical() {
    let data: Vec<u8> = b"The quick brown fox jumps over the lazy dog."
        ...
        .collect();
    for (name, algo) in all_algos() {
        let cmds = algo(&data, &data, 2, TABLE_SIZE, false, false);
...
fn test_empty_version() {
    for (name, algo) in all_algos() {
        let cmds = algo(b"hello", b"", 2, TABLE_SIZE, false, false);
...
fn test_empty_reference() {
    let v = b"hello world";
    for (name, algo) in all_algos() {
        assert_eq!(apply_delta(b"", &algo(b"", v, 2, TABLE_SIZE, false, false)), v, "failed for {}", name);
    }
}
...
fn test_backward_extension() {
    ...
    for (name, algo) in all_algos() {
        assert_eq!(apply_delta(&r, &algo(&r, &v, 4, TABLE_SIZE, false, false)), v, "failed for {}", name);
    }
}
...
fn test_standard_not_detected_as_inplace() {
    ...
    let v: Vec<u8> = b"EFGHABCD"
        ...
        .copied()
        .collect();
    let cmds = diff_greedy(&r, &v, 2, TABLE_SIZE, false, false);
    let placed = place_commands(&cmds);
...
fn test_localmin_picks_smallest() {
    ...
    let cmds = diff_greedy(&r, &v, 4, TABLE_SIZE, false, false);
...
fn test_correcting_checkpointing_tiny_table() {
    ...
    let cmds = diff_correcting(&r, &v, 16, 7, 256, false, false);
...
fn test_correcting_checkpointing_various_sizes() {
    ...
    for q in [7, 31, 101, 1009, TABLE_SIZE] {
        let cmds = diff_correcting(&r, &v, 16, q, 256, false, false);
        let recovered = apply_delta(&r, &cmds);
