...
static DiffOptions opts(size_t p) {
    DiffOptions o;
    o.p = p;
    return o;
}
...
static std::vector<uint8_t> roundtrip(DiffFn algo_fn,
    ...
    auto delta_bytes = encode_delta(placed, false, output_size(cmds), src_c, dst_c);
...
static std::vector<uint8_t> inplace_binary_roundtrip(DiffFn algo_fn,
    ...
    auto src_c = crc64_xz(r.data(), r.size());
    auto dst_c = crc64_xz(v.data(), v.size());
    auto delta_bytes = encode_delta(ip, true, v.size(), src_c, dst_c);
    auto [ip2, is_ip, vs, sc, dc] = decode_delta(delta_bytes);
    REQUIRE(sc == src_c);
    REQUIRE(dc == dst_c);
...
TEST_CASE("identical strings produce only copies", "[integration]") {
    ...
    for (auto& [name, algo] : all_algos()) {
        auto cmds = algo(data, data, opts(2));
...
TEST_CASE("binary encoding roundtrip", "[integration]") {
    ...
    std::array<uint8_t, DELTA_CRC_SIZE> src_c{}, dst_c{};
    src_c.fill(0xAB);
    dst_c.fill(0xCD);
    auto encoded = encode_delta(placed, false, 491, src_c, dst_c);
    auto [decoded, is_ip, vs, sc, dc] = decode_delta(encoded);
    CHECK_FALSE(is_ip);
    CHECK(vs == 491);
    CHECK(sc == src_c);
    CHECK(dc == dst_c);
...
TEST_CASE("binary encoding magic v3", "[integration]") {
...
TEST_CASE("backward extension", "[integration]") {
    ...
    v.insert(v.end(), {'*','*'});
    for (auto& [name, algo] : all_algos()) {
        auto result = apply_delta(r, algo(r, v, opts(4)));
        REQUIRE(result == v);
    }
}
...
TEST_CASE("standard not detected as inplace", "[inplace]") {
    ...
    auto src_c = crc64_xz(r.data(), r.size());
    auto dst_c = crc64_xz(v.data(), v.size());
    auto delta_bytes = encode_delta(placed, false, v.size(), src_c, dst_c);
